...
func deptCreateSQL(req request.DeptCreateRequest, username string) (string, []interface{}, error) {
	return SQLBuilder().Insert(TBNameRole).
		Columns("parent_id", "dept_name", "dept_code", "order_num", "create_time", "create_by", "remark").
		Values(req.ParentID, req.DeptName, req.DeptCode, req.OrderNum, time.Now(), username, req.Remark).
		Suffix("RETURNING \"id\"").
		ToSql()
}
...
func deptPageAndKeywordSQL(req request.PaginationRequest) (querySQL, countSQL string, args []interface{}, err error) {
...
func (store *SQLStore) DeptPage(ctx context.Context, req request.PaginationRequest) (int64, []AgoDept, error) {
	...
	pageSQL, countSQL, args, err := deptPageAndKeywordSQL(req)
	if err != nil {
		return fail(err)
	}
...
func (store *SQLStore) DeptList(ctx context.Context) ([]AgoDept, error) {
	...
	sql, args, err := SQLBuilder().Select("*").From(TBNameDept).Where(sq.And{
		sq.Eq{"status": "0"},
		sq.Eq{"del_flag": "N"},
	}).ToSql()
	if err != nil {
