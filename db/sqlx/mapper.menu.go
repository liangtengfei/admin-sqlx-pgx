...
func menuPageAndKeywordSQL(req request.PaginationRequest) (querySQL, countSQL string, args []interface{}, err error) {
...
func (store *SQLStore) MenuPage(ctx context.Context, req request.PaginationRequest) (int64, []AgoMenu, error) {
	...
	pageSQL, countSQL, args, err := menuPageAndKeywordSQL(req)
	if err != nil {
		return fail(err)
	}
